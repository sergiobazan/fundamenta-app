...
import { CompanyAnalysisProgress } from "@/components/CompanyAnalysisProgress";
import { CompanyAnalysisStatusBanner } from "@/components/CompanyAnalysisStatusBanner";
import { EventTimeline } from "@/components/EventTimeline";
import { getCompany, getCompanyAnalysis, getEvents, getFilings, getNotes, getSummary, statementNames } from "@/lib/financial";
...
export default async function CompanyPage({ params }: { params: Promise<{ smvRpj: string }> }) {
  ...
  let catalogCompany;
  try {
    catalogCompany = await getCompany(smvRpj);
  } catch {
    notFound();
  }
  if (!catalogCompany.has_analysis) {
    const analysis = await getCompanyAnalysis(smvRpj);
    return <CompanyAnalysisProgress initial={analysis} />;
  }

  const analysisYear = catalogCompany.latest_fiscal_year;
  const analysisScope = catalogCompany.preferred_scope;
  if (!analysisYear || !analysisScope) notFound();
  let summary;
  let filings;
  let events;
  ...
  try {
    [summary, filings, events, notes] = await Promise.all([
      getSummary(smvRpj, analysisYear, analysisScope),
      getFilings(smvRpj),
      getEvents({ companyRpj: smvRpj, limit: 3 }),
      getNotes(smvRpj, { year: analysisYear, scope: analysisScope }).catch(() => null),
    ]);
  } catch {
    notFound();
  }
  ...
  const notesHref = `/empresas/${smvRpj}/notas?year=${summary.period.year}&scope=${summary.period.scope}`;
  ...
  return <>
    ...
    <header className="company-detail-header"><div className="company-mark large">{companyMark(summary.company.legal_name)}</div><div><span className="overline">SMV {summary.company.smv_rpj} · RUC {summary.company.ruc}</span><h1>{summary.company.legal_name}</h1><p>{summary.company.sector || "Sector sin clasificar"} · Información anual {summary.period.scope === "consolidated" ? "consolidada" : "individual"}</p></div>{sourceUrl&&<a className="document-button" href={sourceUrl} target="_blank" rel="noreferrer">Documento original <span>↗</span></a>}</header>
    {catalogCompany.analysis_status !== "available" && <CompanyAnalysisStatusBanner initial={catalogCompany} />}
    <AnnualComparisonBanner title={`${summary.company.legal_name} · Métricas financieras`} year={summary.period.year} scope={summary.period.scope} href={`/empresas/${smvRpj}/comparar?year=${summary.period.year}&scope=${summary.period.scope}`} sourceUrl={sourceUrl} />
    <section className="company-facts"><div><span>Periodo analizado</span><b>{summary.period.year} · Anual</b></div><div><span>Estados disponibles</span><b>{currentFilings.length} de 3</b></div><div><span>Indicadores calculados</span><b>{summary.metrics.filter((metric) => metric.status === "computed").length} de {summary.metrics.length}</b></div><div><span>Calidad</span><b className="quality-text">● {catalogCompany.validation_tier === "verified" ? "Verificada manualmente" : passedValidations === currentFilings.length ? "Validación automática" : "Parcial"}</b></div></section>

    <section className="company-section"><div className="content-head"><div><span className="overline">ESTADOS FINANCIEROS</span><h2>Explora las cuentas reportadas</h2><p>Cada vista conserva etiqueta original, concepto normalizado, moneda, escala y fuente.</p></div></div><div className="statement-cards">{currentFilings.map((filing) => <Link href={`/empresas/${smvRpj}/estados/${filing.statement_type}?year=${filing.fiscal_year}&scope=${filing.scope}`} key={filing.statement_type}><div className="statement-icon">{filing.statement_type === "balance_sheet" ? "▤" : filing.statement_type === "income_statement" ? "↗" : "≈"}</div><span>{filing.fiscal_year} · {filing.scope === "consolidated" ? "Consolidado" : "Individual"}</span><h3>{statementNames[filing.statement_type]}</h3><p>{filing.mapped_facts} de {filing.facts} cuentas normalizadas</p><div className="statement-card-foot"><i className={filing.failed_validations ? "failed" : ""}></i>{filing.failed_validations ? `${filing.failed_validations} alertas` : "Controles aprobados"}<b>→</b></div></Link>)}</div></section>

    {notes && <section className="company-section"><div className="content-head"><div><span className="overline">NOTAS AUDITADAS</span><h2>El contexto detrás de las cifras</h2><p>{notes.document.notes_count} notas extraídas y referenciadas por página desde el PDF oficial.</p></div><Link href={notesHref}>Explorar todas las notas ↗</Link></div><Link className="company-notes-callout" href={notesHref}><div><span>Documento {notes.document.scope === "consolidated" ? "consolidado" : "individual"} · {notes.document.fiscal_year}</span><h3>Políticas, estimaciones, deuda, contingencias y hechos posteriores</h3><p>{notes.notes.filter((note) => note.is_priority).length} notas marcadas como relevantes para comenzar el análisis.</p></div><b>Leer notas →</b></Link></section>}
