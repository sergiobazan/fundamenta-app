...
export default async function StatementPage({ params }: { params: Promise<{ smvRpj: string; statementType: string }> }) {
  const { smvRpj, statementType } = await params;
  if (!validTypes.has(statementType as Filing["statement_type"])) notFound();
  let statement;
  try {
    statement = await getStatement(smvRpj, statementType as Filing["statement_type"]);
  } catch {
    notFound();
  }
